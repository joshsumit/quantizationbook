...
This mismatch point is a boundary. At every boundary, the values must be converted from one domain to another — rescaled, rounded, and clamped. Boundaries can only disappear when operators are *fused* — merged into a single combined operation so the conversion happens once at the chain’s output rather than at each intermediate step. (Fusion is covered in Chapter 8.) Boundaries can also disappear when multiple operators intentionally share a single domain.
