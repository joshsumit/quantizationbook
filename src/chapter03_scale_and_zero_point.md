...
A quantization grid provides a fixed number of integer levels spanning a target range. However, given a continuous range of real valuesâ€”for example, activations distributed between -2.3 and 5.1â€”and a discrete budget of 256 integers (0 through 255), we must establish a deterministic rule to map real values to specific integers.
...
Scale (\\(S\\)) is the real-valued width of one quantization step â€” the distance between two adjacent representable values. It is the step size from Chapter 2 made precise.
...
Each integer step corresponds to a real-valued increment of 0.02902. A smaller scale means finer resolution but a narrower range. A larger scale means coarser resolution but a wider range. This is the rangeâ€“precision trade-off from Chapter 2, now expressed as a single number.
...
Dequantized: \\(0.02902 \times (207 - 79) = 0.02902 \times 128 = 3.715\\). The original value was 3.7; the reconstructed value is 3.715. The difference â€” 0.015 â€” is the rounding error for this specific value, well within one step size of 0.029.
...
The step size is 0.04016, compared to 0.02902 under asymmetric. Resolution is 38% coarser. And the range below -2.3 â€” from -5.1 to -2.3 â€” contains no actual data. Roughly 27% of the grid points represent values that never occur.

**Concrete comparison â€” mapping three values through both schemes:**
...
When two quantized values are multiplied and accumulated â€” the core operation in every linear layer and convolution â€” the full integer arithmetic expands as follows. Let \\(q_w = \text{round}(w/S_w) + Z_w\\) and \\(q_x = \text{round}(x/S_x) + Z_x\\). The real-valued dot product \\(\sum w_i x_i\\) becomes:

$$S_w S_x \left[\sum q_{w_i} q_{x_i} - Z_x \sum q_{w_i} - Z_w \sum q_{x_i} + N \cdot Z_w Z_x\right]$$

The integer dot product is only the first term; zero-points introduce three additional correction terms. The first term â€” \\(\sum q_{w_i} q_{x_i}\\) â€” is the integer dot product that hardware accelerates. The remaining three are *correction terms* introduced by the zero-points:
...
These corrections add two extra vector reductions and a scalar add to *every single matrix multiply in the model*. For a [4096 Ã— 4096] weight matrix, each output row requires summing 4096 quantized weights and 4096 quantized inputs â€” per output element, per layer, per inference.
...
**Weights â†’ Symmetric.** Neural network weights are typically distributed roughly symmetrically around zero â€” a bell curve centered at (or very near) 0.0. Symmetric quantization fits this distribution naturally without wasting codes. More importantly, setting \\(Z_w = 0\\) eliminates two of the three correction terms in the dot-product expansion above: the \\(Z_w \sum q_{x_i}\\) term (which depends on the input and must be computed at runtime) and the \\(N \cdot Z_w Z_x\\) constant both vanish. Only the \\(Z_x \sum q_{w_i}\\) term remains â€” and \\(\sum q_{w_i}\\) depends only on weights, so it can be precomputed once during model preparation and stored as a constant. This makes the per-inference cost of the remaining correction nearly free.

**Activations â†’ Asymmetric.** Activations after ReLU are strictly non-negative: their range is \\([0, r_{\max}]\\). If you apply symmetric quantization, the range becomes \\([-r_{\max}, +r_{\max}]\\), and *half* of your 256 int8 codes represent negative values that can never occur â€” you are left with effectively ~128 usable levels instead of 256. Asymmetric quantization maps the zero-point to code 0 (since the minimum is 0.0), giving you all 256 codes for the actual \\([0, r_{\max}]\\) range. For non-ReLU activations (GELU, SiLU, etc.) the distribution may extend below zero, in which case the advantage is smaller â€” but even there, activations are rarely perfectly symmetric, so asymmetric usually fits better.
...
One correction term instead of three. And \\(\sum q_{w_i}\\) is a precomputed constant per output channel â€” so the runtime cost is one extra addition per output element, which hardware handles trivially.

This is why Appendix B uses exactly this setup: asymmetric uint8 for activations, symmetric int8 for weights. It is not an arbitrary choice â€” it is the best trade-off between representational precision (asymmetric activations waste no codes) and compute efficiency (symmetric weights eliminate runtime correction overhead).
...
In static quantization, they cannot be adjusted at inference time â€” they cannot be tuned per-input. (Dynamic quantization, covered later, recomputes activation scales per input or per batch, but weight scales remain fixed even there.) If the scale is wrong â€” if it was computed from unrepresentative data, or if the true range of values extends beyond what the scale covers â€” every inference will carry that error. Their permanence is the point: scale and zero-point are binding contracts between the quantization scheme and every computation that follows.
...
Whenever you encounter a quantized layer, two questions should be immediate: What is the scale â€” and is it fine enough to preserve the distinctions this layer depends on? What is the zero-point â€” and does zero map exactly?

This chapter defined the mapping for one tensor with one scale and one zero-point. In practice, different output channels can have very different value distributions, and a single scale is a blunt instrument. Per-channel scales and groupwise quantization â€” covered in later chapters â€” address this by giving each channel or group its own contract.
