...
>   â”‚
>   â”œâ”€ Is it SLOWER than float? (throughput problem)
>   â”‚     â”œâ”€ Check backend logs for dequant pairs â†’ Silent Fallback (Pattern 3)
>   â”‚     â””â”€ Check kernel profiler for unfused ops â†’ Fusion Loss (Ch.8)
>   â”‚
>   â””â”€ Is it LESS ACCURATE? (quality problem)
>         â”‚
>         â”œâ”€ Uniform degradation across all inputs?
>         â”‚     â”œâ”€ Check activation histograms for outlier channels â†’ Outlier Explosion (Pattern 1)
>         â”‚     â””â”€ Check per-layer sensitivity â†’ Resolution Collapse / Scale Mismatch (Pattern 5)
>         â”‚
>         â”œâ”€ Degradation only on production data (not calibration)?
>         â”‚     â””â”€ Calibration Drift (Pattern 4)
>         â”‚
>         â”œâ”€ Depth-dependent degradation (deeper = worse)?
>         â”‚     â””â”€ Residual Ghost (Pattern 2) â€” check scale alignment at skip connections
>         â”‚
>         â””â”€ Nothing works (PTQ, QAT, all fail)?
>               â””â”€ Unquantizable Model (Pattern 6) â€” raise precision floor
> ```
>
...
**Mechanism:** Consider a layer where 99.9% of activations fall in [-1.0, 3.0] but one channel produces values reaching 50.0. If the observer uses min-max (Chapter 9), the scale is set for [-1.0, 50.0] â€” a range of 51 units. The step size is \\(51/255 \approx 0.20\\). Values in the common range [-1.0, 3.0] get approximately 20 grid points. The layer effectively operates at 4â€“5 bit resolution instead of 8 bit.

**Diagnostic question:** What is the ratio between the 99.9th percentile and the maximum activation value? The *99.9th percentile* (written p99.9) is the value below which 99.9% of observations fall â€” it is robust to single extreme outliers. A p99.9/max ratio below 0.1 (i.e., the maximum is more than 10Ã— the 99.9th percentile) strongly suggests outlier explosion.

**Fix / Mitigation:**

- Switch the minâ€“max observer to a percentile or histogram observer (Chapter 9) â€” this trades a small amount of clipping for much better resolution in the common range.
...
**Mechanism:** In a ResNet with 25 residual blocks, each block contributes one error-merging site. If the main and residual paths have mismatched scales, each merge point also contributes a rescale boundary. Over 25 blocks, the accumulated error from independently quantized paths reshapes the representation â€” particularly in the higher layers where the model relies on subtle activation differences.
...
**Symptom:** The quantized model runs slower than expected. In some cases, it is slower than the floating-point original. Accuracy may be fine â€” the failure is in throughput.
...
**Mechanism:** A model uses GELU activation, which is not in the int8 capability envelope of the target backend. At each GELU layer, the runtime dequantizes int8 to float32, computes GELU in float, and requantizes back to int8. Each fallback adds two memory-format conversions and two requantization boundaries. If GELU appears after every linear layer in a 24-layer transformer, that is 48 additional boundaries and 48 additional memory round trips â€” none of which were in the quantization plan.

**Concrete latency cost.** For a [1, 4096] activation tensor in int8 (4 KB), each fallback boundary requires: dequantize int8 â†’ float32 (4 KB â†’ 16 KB, kernel launch ~5 Âµs), GELU compute in float32 (~20 Âµs), requantize float32 â†’ int8 (16 KB â†’ 4 KB, ~5 Âµs). Per-layer fallback cost: ~30 Âµs. If the fused int8 Linear-ReLU alternative takes ~25 Âµs for compute, the GELU fallback adds 5 Âµs overhead per layer. Across 24 layers: \\(24 \times 5 = 120\\) Âµs extra per inference. For a model that should complete inference in 2 ms, this is a 6% slowdown from a single unsupported activation function â€” and the model appears fully quantized in the graph.
...
- Track saturation rate and range exceedance in production telemetry â€” this is the actionable invariant.
...
**Symptom:** Unexpected additional requantization boundaries appear in the execution graph â€” more than the layer count would predict. Latency is higher than expected, and accuracy drops are concentrated at specific points in the graph.
...
**Mechanism:** A model with a feature pyramid network concatenates feature maps from different stages. Each stage has been quantized with different scales. The concatenation requires all inputs to share one \\((S, Z)\\). Rescale operations are inserted on two of the three input branches, adding two boundaries at each pyramid level. Across four pyramid levels, eight additional boundaries appear â€” each introducing rounding error.
...
- Narrow hidden dimension (1024) â€” meaning each dot product has only 1024 terms, giving less averaging to smooth out per-weight quantization error
...
| QAT | int8 | 1.5% | Acceptable â€” but QAT cost 3 GPU-days |
| PTQ | int4 | 38% | Total collapse |
| GPTQ | int4 | 18% | Still incoherent |
| AWQ | int4 | 15% | Marginally better, still broken |

**The diagnosis:** At 350M parameters, the model is too small for aggressive quantization. Its representations are already compact â€” there is less redundancy for quantization to exploit. Each weight carries more information per parameter than in a 70B model (a heuristic, not a theorem â€” architecture matters as much as size), so the same per-weight error has a larger relative impact.

**The correct answer:** Do not quantize this model below int8 without QAT. And do not quantize to int4 at all â€” at some point the precision floor is reached; beyond it, even advanced methods cannot reliably recover accuracy for a model this compact.

**The signal to look for:** When *every* quantization approach at a given precision produces unacceptable accuracy drops, the model is telling you that the precision floor has been reached. The fix is not a better algorithm â€” it is a different precision or a different (larger) model whose representations are more redundant and therefore more compressible.
...
- If economics allow, use QAT â€” as the table above shows, it can recover int8 accuracy at the cost of training compute (Chapter 11).

---

## The Standard Model vs. The Transformer Reality

The five failure patterns above â€” and the diagnostic machinery that identifies them â€” were developed against convolutional networks and feedforward architectures. These models have smooth activation distributions, bounded value ranges, and well-behaved weight statistics. The quantization framework from Chapters 1â€“12 handles them well, and the failure patterns above capture the standard ways things go wrong.

Transformers rewrite the rules. Large language models and vision transformers produce activation distributions with *structural* outliers â€” not random extremes, but specific channels that consistently fire at 30â€“100Ã— the magnitude of their neighbors. These outliers are not noise to be clipped or calibrated around. They are a fundamental property of the architecture.

The standard observer (Chapter 9) cannot fix this â€” no choice of percentile eliminates outliers that are structurally necessary. PTQ (Chapter 10) collapses entirely. QAT (Chapter 11) is theoretically possible but economically prohibitive for 70-billion-parameter models. The diagnostic patterns above still apply, but the dominant failure mode in transformers â€” outlier explosion at architectural scale â€” requires a different class of solutions entirely.
...
Quantization failures have names. Outlier explosion, residual ghost, silent fallback, calibration drift, scale mismatch, and unquantizable-model behavior are not exotic edge cases â€” they are the standard failure modes that account for the vast majority of quantization accuracy and performance regressions.
